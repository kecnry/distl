### [Histogram](Histogram.md).plot_dist (method)
...
* `**kwargs`: all keyword arguments will be passed on to plt.plot.  Note:
    if wrapping is enabled, either via `wrap_at` or [Histogram.wrap_at](Histogram.wrap_at.md),
    ...
    matplotlib behavior.  Calling this through [Histogram.plot](Histogram.plot.md) with
